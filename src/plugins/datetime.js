...
export default {
  ...
  install: Vue => {
    const DatetimeService = {
       ...
       * @returns { String Date}
       */
      transformFormat(date, fromFormat = 'DD/MM/YYYY', toFormat = 'YYYY-MM-DD') {
        let sDate;
      ...
      /**
       * @author dbarra
       * @description Retorna HH:MM:SS
       * @returns { Number }
       * @param { date } date tiene que ser una fecha validada por ejemplo usando new Date()
       */
      getTime (date) {
        return `${this.getHour(date)}:${this.getMinutes(date)}:${this.getSeconds(date)}`;
      }
    };
