              ...
              >
                <template v-for="(filter, idx) in filters">
                  ...
                  <v-input-select
                    ...
                    :init="search[filter.field]"
                    class="ml-2 mr-2 mb-1"
                    :class="filter.nowrap ? 'wrap--component' : ''"
                    :label="filter.label"
                    :items="filter.values"
                    item-text="text"
                    item-value="value"
...
export default {
  ...
  methods: {
    ...
    async getData(refresh = false) {
      try {
        if ((this.url || '').includes('null') || (this.url || '').includes('null') || (this.url || '').length === 0) {
        ...
        if (this.filtroAsQuery) {
          ...
          for (const key in this.search) {
            if (typeof this.search[key] !== 'function' && this.search[key] !== undefined) {
              searchFiltro[key] = this.search[key].toString().trim();
            }
          }
          query = {
            ...query,
    ...
    setPropsDataTable(loading = false, items = [], count = 0) {
      ...
      this.$store.commit('setTotalCurrentCrudTable', count);
    },
    filtrar() {
      // eslint-disable-next-line vue/no-mutating-props
