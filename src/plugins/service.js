  ...
  install: (Vue, config) => {
    ...
    const Services = {
      ...
      file(
        ...
        method = 'GET',
        data = {},
        nameFile = new Date().toLocaleDateString()
      ) {
        try {
          return new Promise(async (resolve, reject) => {
            const response = await request({
              ...
              data,
              timeout: 0,
              responseType: ['ods', 'xls', 'pdf', 'exe'].includes(extension) ? 'blob' : '',
              withCredentials: true,
              headers: {
                Authorization: `${authToken} ${Storage.get('token')}`
              }
    ...
    request.interceptors.response.use(
      response => response,
      error => {
        if (!error.response && error.message?.includes('timeout') && error.message?.includes('exceeded')) {
          Waiting(false);
          Message.error('Tiempo de espera excedido. Intente mas tarde');
          return {
            status: 408,
          };
        }
