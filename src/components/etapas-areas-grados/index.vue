<template>
  <v-card elevation="0">
          ...
          <v-container fluid v-if="modalEtapaAreaGrado">
            <v-stepper v-model="pasoActual" class="elevation-0">
              <v-stepper-header class="elevation-0">
                <v-stepper-step :complete="pasoActual > 1 && !form.id" step="1" :editable="form.id ? true : false">
                  Información
                </v-stepper-step>

                <v-divider></v-divider>

                <v-stepper-step :complete="pasoActual > 2 && !form.id" step="2" :editable="form.id ? true : false">
                  Preguntas
                </v-stepper-step>

                <v-divider></v-divider>

                <v-stepper-step :complete="pasoActual > 3 && !form.id" step="3" :editable="form.id ? true : false">
                  Medallero
                </v-stepper-step>

                <v-divider></v-divider>

                <v-stepper-step :complete="pasoActual > 4 && !form.id" step="4" :editable="form.id ? true : false">
                  Clasificación
                </v-stepper-step>
              </v-stepper-header>
              <v-stepper-items>
                    ...
                    <v-row>
                      <v-col cols="12">
                        ...
                        <v-card class="pa-4" outlined rounded>
                            ...
                            <v-col cols="12" md="12" xs="12" sm="12" lg="6">
                              <v-text-field
                                ...
                                oninput="
                                  ...
                                  if(this.value >= 10080) this.value = '10080';"
                              >
                                <template v-slot:prepend>
                                  <v-icon-component
                                    icon="timer"
                          ...
                          <v-slider
                            :disabled="parseInt(form.preguntasOlimpiada) === parseInt(form.totalPreguntas) || parseInt(form.preguntasOlimpiada) === 0"
                        ...
                        <v-row>
                          ...
                          <v-col>
                            <v-icon-tooltip
                                color="error"
                                icon="speed"
                                text="Puntos en total - Dificultad compleja"
                            />
                      ...
                      <v-col cols="7">
                        <v-row>
                          ...
                          <v-text-field
                              color="primary"
                              type="number"
                              :rules="[ v => parseInt(v) >= 1 || 'Debe ser mayor o igual a 1']"
                                ...
                                <v-tooltip bottom color="success">
                                  <template v-slot:activator="{ on, attrs }">
                                    <v-btn
                                        ...
                                        v-bind="attrs"
                                        v-on="on"
                                        :disabled="deshabilitarAgregarMedallero"
                                        @click="addOption()"
                                    >
                                      <v-icon-component
                                          icon="add"
                          ...
                          <v-card elevation="0">
                            ...
                            >
                              <v-row
                                  ...
                                  class="ma-2"
                              >
                                <v-col cols="3" class="primary--text ma-2">
                                <span class="font-weight-medium grey--text">
                                ...
                                <v-col>
                                  <v-opciones-medallero
                                      :key="opcion.key"
                                      :id="opcion.key"
                                      :orden-galardon-inicial="opcion.ordenGalardon.toString()"
                                      :denominativoInicial="opcion.denominativo"
                                      :sub-grupo-inicial="opcion.subGrupo"
                              ...
                              >
                                ...
                                <v-col>
                                  <v-opciones-medallero
                                      ...
                                      :minimoInicial="opcion.posicionMinima"
                                      :maximoInicial="opcion.posicionMaxima"
                                  ...
                                  "
                                      ...
                                      :total-medalleros="medalleroPosiciones.length"
                                  />
                                </v-col>
                              </v-row>
                              <div
                                      ...
                                      <v-col cols="7">
                                        <v-row>
                                          ...
                                          <v-text-field
                                              color="primary"
                                              type="number"
                                              :rules="[v => parseInt(v) >= 0 || `Debe ser mayor o igual 0`]"
                                              v-model="
                                  ...
                                  <v-col cols="7">
                                    <v-row>
                                      <v-icon-tooltip
                                          color="primary"
                                          icon="info"
                                          :text="'Debe ser mayor o igual a 1'"
                                      />
                                      <v-text-field
                                          color="primary"
                                          required
                                          type="number"
                                          :rules="[v => v === '' || v === null || parseInt(v) >= 1 || 'Debe ser mayor o igual a 1']"
    ...
    >
      <template slot="buttons">
        <v-icon-tooltip
          ...
          v-if="btnAgregar && etapaSeleccionada != null && etapaSeleccionada.estado === 'CONFIGURACION_GRADOS'"
          @accion="oModal({})"
        />
      </template>
      ...
      <template slot="items" slot-scope="items">
        ...
        >
          ...
          <td>
            ...
            <span v-if="![ESTADO_CONFIGURACION_GRADOS, ESTADO_EXAMEN_SEGUN_CRONOGRAMA ,ESTADO_SORTEO_PREGUNTAS,  ESTADO_HABILITACION_REZAGADOS].includes(items.items.etapa.estado)" class="font-weight-regular">
            ...
            <v-icon-tooltip
                ...
                icon="toggle_on"
                text="Inactivar"
                v-if="btnEditar && items.items.estado === 'ACTIVO' && items.items.etapa.estado === ESTADO_CONFIGURACION_GRADOS"
            ...
            <v-icon-tooltip
                ...
                icon="toggle_off"
                text="Activar"
                v-if="btnEditar && items.items.estado === 'INACTIVO' && items.items.etapa.estado === ESTADO_CONFIGURACION_GRADOS"
            ...
            <v-icon-tooltip
                ...
                v-if="btnEditar && items.items.etapa.estado === ESTADO_CONFIGURACION_GRADOS"
                @accion="oModal(items)"
            />
            ...
            <v-icon-tooltip
              color="info"
              icon="date_range"
              :text="[ESTADO_CONFIGURACION_COMPETENCIA, ESTADO_HABILITACION_REZAGADOS].includes(items.items.etapa.estado) ? 'Configurar calendario' : 'Ver todos los calendarios'"
              v-if="![ESTADO_ACTIVO, ESTADO_CONFIGURACION_GRADOS].includes(items.items.etapa.estado)"
              @accion="setCalendar(items)"
            />
          </td>
        </tr>
      </template>
    ...
    <v-dialog v-model="$store.state.modal3" width="1200" persistent>
      <v-card elevation="0">
            ...
            <v-row no-gutters>
              ...
              >
                ...
                <span class="fs10 pl-2">
                  Calendarios para la etapa
                  <strong>{{ (etapaSeleccionada && etapaSeleccionada.nombre ? etapaSeleccionada.nombre : '' || '').toLowerCase() }}</strong>
                  del area de <strong>{{ (etapaAreaGradoSeleccionado && etapaAreaGradoSeleccionado.area ? etapaAreaGradoSeleccionado.area.nombre : '' || '').toLowerCase() }}</strong>
                </span>
              </v-col>
        ...
        <v-card-text class="pa-3 pt-0">
          <calendar
            ...
            :ESTADO_CONFIGURACION_COMPETENCIA="estadoCalendario === ESTADO_HABILITACION_REZAGADOS ? ESTADO_HABILITACION_REZAGADOS : ESTADO_CONFIGURACION_COMPETENCIA"
            :estado="estadoCalendario"
            :etapaAreaGrado="etapaAreaGradoSeleccionado"
            :area="form.id"
            reference="calendar"
          />
        </v-card-text>
...
const ESTADO_CONFIGURACION_COMPETENCIA = 'CONFIGURACION_COMPETENCIA';
const ESTADO_CONFIGURACION_GRADOS = 'CONFIGURACION_GRADOS';
const ESTADO_HABILITACION_REZAGADOS = 'HABILITACION_REZAGADOS';
const ESTADO_ACTIVO = 'ACTIVO';
const ESTADO_SORTEO_PREGUNTAS = 'SORTEO_PREGUNTAS';
const ESTADO_EXAMEN_SEGUN_CRONOGRAMA = 'EXAMEN_SEGUN_CRONOGRAMA';
...
export default {
  ...
  watch: {
    'form.totalPreguntas'() {
      if (this.indicadorModalAbierto) {
        ...
        if (parseInt(this.form.totalPreguntas) === 1) {
          this.form.puntosPreguntaCurricula = parseInt(this.form.preguntasOlimpiada) === 0 ? parseInt(this.form.puntajeTotal) : 0;
        } else {
          this.form.puntosPreguntaCurricula = parseInt(this.form.puntajeTotal / 2);
        }
        this.form.puntosPreguntaOlimpiada = this.form.puntajeTotal - this.form.puntosPreguntaCurricula;
      }
    },
    'form.preguntasOlimpiada'(newValue, oldValue) {
      if (this.indicadorModalAbierto) {
        ...
        if (parseInt(this.form.preguntasOlimpiada) === parseInt(this.form.totalPreguntas)) {
          this.form.puntosPreguntaOlimpiada = parseInt(this.form.puntajeTotal);
        }

        if (parseInt(this.form.preguntasOlimpiada) === 0) {
          this.form.puntosPreguntaOlimpiada = 0;
        }
        if (parseInt(oldValue) === parseInt(this.form.totalPreguntas) || oldValue === 0) {
          if (parseInt(this.form.totalPreguntas) === 1) {
            this.form.puntosPreguntaCurricula = parseInt(this.form.preguntasOlimpiada) === 0 ? parseInt(this.form.puntajeTotal) : 0;
          } else {
            this.form.puntosPreguntaCurricula = parseInt(this.form.puntajeTotal / 2);
    ...
    'form.puntosPreguntaOlimpiada'() {
      if (this.indicadorModalAbierto) {
        ...
        if (parseInt(this.form.puntosPreguntaOlimpiada) === 0) {
          this.form.preguntasOlimpiada = 0;
        }

        if (parseInt(this.form.puntosPreguntaOlimpiada) === parseInt(this.form.puntajeTotal)) {
          this.form.preguntasOlimpiada = parseInt(this.form.totalPreguntas);
        }
      }
    },
  ...
  data: () => ({
    ...
    form: {
      ...
      puntajeMinimoMedallero: 1,
      nroPosicionesRural: 0,
      criterioCalificacion: false,
      criterioMedallero: false,
      puntajeMinimoClasificacion: 0,
      cantidadMaximaClasificados: null,
    },
    max_numero_preguntas: 1000,
    max_tiempo_prueba: 10080,
    medalleroPosiciones: [],
    medalleroPosicionRurales: [],
    filters: [],
    ...
    ESTADO_CONFIGURACION_COMPETENCIA: ESTADO_CONFIGURACION_COMPETENCIA,
    ESTADO_ACTIVO: ESTADO_ACTIVO,
    ESTADO_CONFIGURACION_GRADOS: ESTADO_CONFIGURACION_GRADOS,
    ESTADO_HABILITACION_REZAGADOS: ESTADO_HABILITACION_REZAGADOS,
    ESTADO_SORTEO_PREGUNTAS: ESTADO_SORTEO_PREGUNTAS,
    ESTADO_EXAMEN_SEGUN_CRONOGRAMA: ESTADO_EXAMEN_SEGUN_CRONOGRAMA
  }),
  mounted() {
    this.$nextTick(async () => {
      ...
      if (this.$Casbin && this.$Casbin.enforcer) {
        ...
        this.btnAgregar = await this.$Casbin.enforcer.enforce(rol, '/etapas-areas-grados', 'create');
        this.btnEditar = await this.$Casbin.enforcer.enforce(rol, '/etapas-areas-grados', 'update');
        this.btnEliminar = await this.$Casbin.enforcer.enforce(rol, '/etapas-areas-grados', 'delete');
      }
      ...
      this.urlAcciones = 'etapasAreaGrado';
    });
  },
  computed: {
    deshabilitarAgregarMedallero() {
      return this.medalleroPosiciones.filter(medallero => medallero.denominativo === '' || medallero.subGrupo === '').length > 0;
    }
  },
  methods: {
    obtenerRuta() {
    ...
    setCalendar({ items }) {
      this.etapaAreaGradoSeleccionado = {
        id: items.id,
        area: items.area,
        gradoEscolar: items.gradoEscolar
      };
      this.estadoCalendario = items.etapa.estado;
    ...
    validarPonderadion() {
      this.form.puntajeCurriculaBaja = isFinite(this.form.totalPuntajeCurriculaBaja / this.form.preguntasCurriculaBaja) ? this.form.totalPuntajeCurriculaBaja / this.form.preguntasCurriculaBaja : 0;
      console.log(`this.form.puntajeCurriculaBaja: ${this.form.puntajeCurriculaBaja}`);
      this.form.puntajeCurriculaMedia = isFinite(this.form.totalPuntajeCurriculaMedia / this.form.preguntasCurriculaMedia) ? this.form.totalPuntajeCurriculaMedia / this.form.preguntasCurriculaMedia : 0;
      console.log(`this.form.puntajeCurriculaMedia: ${this.form.puntajeCurriculaMedia}`);
      this.form.puntajeCurriculaAlta = isFinite(this.form.totalPuntajeCurriculaAlta / this.form.preguntasCurriculaAlta) ? this.form.totalPuntajeCurriculaAlta / this.form.preguntasCurriculaAlta : 0;
      console.log(`this.form.puntajeCurriculaAlta: ${this.form.puntajeCurriculaAlta}`);

      this.form.puntajeOlimpiadaBaja = isFinite(this.form.totalPuntajeOlimpiadaBaja / this.form.preguntasOlimpiadaBaja) ? this.form.totalPuntajeOlimpiadaBaja / this.form.preguntasOlimpiadaBaja : 0;
      console.log(`this.form.puntajeOlimpiadaBaja: ${this.form.puntajeOlimpiadaBaja}`);
      this.form.puntajeOlimpiadaMedia = isFinite(this.form.totalPuntajeOlimpiadaMedia / this.form.preguntasOlimpiadaMedia) ? this.form.totalPuntajeOlimpiadaMedia / this.form.preguntasOlimpiadaMedia : 0;
      console.log(`this.form.puntajeOlimpiadaMedia: ${this.form.puntajeOlimpiadaMedia}`);
      this.form.puntajeOlimpiadaAlta = isFinite(this.form.totalPuntajeOlimpiadaAlta / this.form.preguntasOlimpiadaAlta) ? this.form.totalPuntajeOlimpiadaAlta / this.form.preguntasOlimpiadaAlta : 0;
      console.log(`this.form.puntajeOlimpiadaAlta: ${this.form.puntajeOlimpiadaAlta}`);

      const preCalculoTotal = parseFloat(this.form.puntajeCurriculaAlta) * parseFloat(this.form.preguntasCurriculaAlta)
          + parseFloat(this.form.puntajeCurriculaMedia) * parseFloat(this.form.preguntasCurriculaMedia)
          + parseFloat(this.form.puntajeCurriculaBaja) * parseFloat(this.form.preguntasCurriculaBaja)
          + parseFloat(this.form.puntajeOlimpiadaAlta) * parseFloat(this.form.preguntasOlimpiadaAlta)
          + parseFloat(this.form.puntajeOlimpiadaMedia) * parseFloat(this.form.preguntasOlimpiadaMedia)
          + parseFloat(this.form.puntajeOlimpiadaBaja) * parseFloat(this.form.preguntasOlimpiadaBaja);

      console.log(`preCalculoTotal: ${preCalculoTotal}`);

      return (
        parseInt(this.form.puntajeTotal) === preCalculoTotal
      );
    },
    paso12() {
      if (this.$refs.form1.validate()) {
        if (this.form.totalPreguntas > 0) {
          this.pasoActual = 2;
        } else {
          this.$message.error('Total de preguntas debe ser mayor o igual a 1');
        }
      } else {
    ...
    paso23() {
      if (this.validarPonderadion()) {
        ...
        this.$message.error(
          'La suma de la multiplicación de los puntos/pregunta y la cantidad de preguntas debe ser igual a 100'
        );
      }
    },
    paso34() {
      if (this.$refs.form3.validate()) {
        if (this.medalleroPosiciones.length > 0) {
          this.pasoActual = 4;
        } else {
          this.$message.error('Debe agregar al menos una posición en el medallero');
        }
      } else {
        this.$message.error('Complete los campos obligatorios');
      }
    ...
    paso4Enviar() {
      if (this.$refs.form4.validate()) {
        if (this.form.criterioMedallero || this.form.criterioCalificacion) {
          this.save();
        } else {
          this.$message.error('Debe seleccionar al menos un criterio de clasificación');
        }
      } else {
        this.$message.error('Complete los campos obligatorios');
      }
    ...
    setOption({
      ...
      this.medalleroPosiciones.filter((option) => {
        if (option.key === key) {
          ...
          option.subGrupo = subGrupo ?? option.subGrupo;
    ...
    addOptionRural() {
      if (
        ...
        this.medalleroPosicionRurales.push({
          ...
          posicionMaxima: this.medalleroPosicionRurales.length === 0 ? 1 : '',
          notaMinima: 1,
        });
      } else {
        this.$message.error(
