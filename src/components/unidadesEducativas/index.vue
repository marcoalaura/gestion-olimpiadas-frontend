<template>
  <v-card elevation="0">
                ...
                <v-col cols="12" md="6" xs="12" sm="12" lg="6">
                  ...
                  >
                    <template slot="contenido">
                      <v-select v-model="form.idDistrito"
                                item-text="nombre"
                                item-value="id"
                                :items="distritos" attach outlined
                                :rules="$validate(['required'])">
    ...
    <crud-table :headers="headers" :url.sync="url" :widthModal="500"
...
export default {
  ...
  methods: {
    ...
    async cargarDistritos() {
      await this.$service.get(`distritos?filtro=idDepartamento:${this.form.idDepartamento ?? this.form.idDepartamento ?? ''},${this.searchTermDistrito != null ? `nombre:${this.searchTermDistrito}` : ''}`)
        .then((resultado) => {
    ...
    async oModal({ items }) {
      if (items && items.id) {
        ...
        this.localidades = [items.localidad];
        this.secciones = [items.seccion];
        this.distritos = [items.distrito];
